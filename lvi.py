...
df_odom = pd.read_csv(dir + "/LVI_final/_slash_lvi_sam_slash_lidar_slash_mapping_slash_odometry.csv")
df_gt = pd.read_csv(dir + "/LVI_final/_slash_dwm1001_slash_tag_slash_tag_slash_position.csv")
...
class Odom_points():
    ...
    def match_counter_points(self, gt_object):
        ...
        if len(self.curve) > len(gt_object.curve):
            ...
            for i in range(308, len(gt_object.curve)):
                distances = []
                indexes_positions = []
                indexes_orientations = []
