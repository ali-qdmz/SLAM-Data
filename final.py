...
def plot_all(gt_obj, odom_obj, match=False):
    ...
    if match:
        ...
        markers_on = [i for i in range(len(gt_obj.x[:trim_end])) if i%50 == 0]
        line, = ax.plot(gt_obj.x[:trim_end], gt_obj.y[:trim_end], 'k-',
        ...
        fig.savefig(final_data + "/ground_truth_trajectory.svg", format='svg', dpi=1000)
        plt.show()
...
def plot_errors(odom_object, ground_truth_object, purge=False):
    ...
    if purge:
        ...
        plt.ylabel('translition error, [meters]', fontSize="large")
        plt.plot(errors)
        plt.show()
