...
for item in directories:
    ...
    d[df['memory_mean'][0]] = item.replace("_final", "").replace("_", "-").upper()
    val.append(df['memory_mean'][0])
    names.append(item.replace("_final", "").upper())
    print(item.replace("_final", "").replace("_", "-").upper(), df['memory_mean'][0])

lists = sorted(d.items()) # sorted by key, return a list of tuples
lists.reverse()
x, y = zip(*lists) # unpack a list of pairs into two tuples

x = list(map(megabyte, x))
print(x[0], y[0])

fig, ax = plt.subplots()

for i, txt in enumerate(x):
    ax.annotate(round(x[i], 2), (y[i], x[i]), xytext=(i+0.5, x[i]+6.0),
...
fig.savefig("memory_usage.png", format='png', dpi=600)
plt.show()
