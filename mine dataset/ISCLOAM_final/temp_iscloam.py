...
df_odom = pd.read_csv(dir + "/ISCLOAM_final/_slash_odom_final.csv")
df_gt = pd.read_csv(dir + "/ISCLOAM_final/_slash_dwm1001_slash_tag_slash_tag_slash_position.csv")
print(len(df_odom), len(df_gt))
...
class Odom_points():
    ...
    def match_counter_points(self, gt_object):
        ...
        if len(self.curve) > len(gt_object.curve):
            print("if")
            points = []
            for i in range(len(gt_object.curve)):
                distances = []
                indexes = []
                for j in range(len(self.curve)):
                    ...
                    indexes.append([self.curve[j][0], self.curve[j][1]])
            ...
            for i in range(len(self.curve)):
                ...
                for j in range(len(gt_object.curve)):
                    ...
                    distances.append(distance)
                    indexes.append([gt_object.curve[j][0], gt_object.curve[j][1]])
                counter_point = indexes[distances.index(min(distances))]
...
def plot_errors(odom_object, ground_truth_object):
    ...
    errors = []
    for i in range(len(odom_object.x)):
        error = math.sqrt((odom_object.x_1[i] - ground_truth_object.x_1[i])**2 + (odom_object.y_1[i] - ground_truth_object.y_1[i])**2)
        errors.append(error)
...
# odom_object.scale_shift_rotate(scale=1, shift_x=shift_x,shift_y=shift_y,rotate=5)
# odom_object.match_counter_points(ground_truth_object)
# odom_object.caclulate_rmse(ground_truth_object)



# plt.plot(odom_object.x, odom_object.y)
# plt.show()
# plot_all(odom_obj=odom_object,gt_obj=ground_truth_object)


cut = 307
plt.plot(ground_truth_object.x[:cut], ground_truth_object.y[:cut])
plt.show()