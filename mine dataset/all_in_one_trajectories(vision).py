...
dir = "/home/ali/bagfiles (copy)/orb_slam_2_final"
fig, ax = plt.subplots(1, 1)
df_gt = pd.read_csv(dir + "/_slash_lio_sam_slash_mapping_slash_odometry.csv")
gt_obj = GT_points(df=df_gt)


markers_on = [i for i in range(len(gt_obj.x[225:])) if i%20 == 0]
line, = ax.plot(gt_obj.x[225:], gt_obj.y[225:], 'k-',
                markevery=markers_on, marker= 'o', color="red")
...
odom_object = Odom_points(df=df_odom, reverse_x=False)
shift_x = gt_obj.x[225] - odom_object.x[425]
shift_y = gt_obj.y[225] - odom_object.y[425]
odom_object.scale_shift_rotate(scale=1, shift_x=shift_x,shift_y=-shift_y,rotate=-4)
plt.plot(odom_object.x[425], odom_object.y[425], marker='o', color = 'green')
