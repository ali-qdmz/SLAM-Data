...
def plot_all(gt_obj, odom_obj, match=False, legend=None, color='black'):
    ...
    if match:
        ...
        # fig, ax = plt.subplots(1, 1)
        line, = ax.plot(odom_obj.x, odom_obj.y, 'k-', label=legend, color=color)
        add_arrow_to_line2D(ax, line, arrow_locs=np.linspace(0., 1., 50),
                            arrowstyle='->')
...
shift_y = ground_truth_object.y[0] - odom_object.y[0]

odom_object.scale_shift_rotate(scale=1, shift_x=shift_x,shift_y=shift_y,rotate=0)
plot_all(odom_obj=odom_object,gt_obj=ground_truth_object, legend='Vins Mono SLAM')
...
plt.ylabel('y, [meters]')
# plt.savefig(final_data + "/ground_truth_trajectory.png")
fig.savefig("VINS.png", format='png', dpi=600)
